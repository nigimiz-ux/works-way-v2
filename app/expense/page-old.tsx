...
export default function ExpensePage() {
  ...
  const fetchMyExpenses = async () => {
    ...
    if (error) {
      console.error("사용 내역 로딩 에러:", error);
    } else {
      setExpenses(data || []);
    }
    setIsLoading(false);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        console.error("보고 에러:", error);
        alert(`보고 중 오류가 발생했습니다: ${error.message}`);
      } else {
        // 텔레그램 알림 전송 (에러가 나도 사용자 프로세스를 막지 않도록 비동기 처리)
        fetch('/api/telegram', {
          method: 'POST',
          headers: {
    ...
    <div className="w-full animate-fade-in flex flex-col min-h-[calc(100vh-6rem)]">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-10">
            ...
            <form onSubmit={handleSubmit} className="space-y-5">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-600 focus:border-transparent transition-all outline-none text-slate-900 dark:text-slate-100"
                />
              </div>
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-600 focus:border-transparent transition-all outline-none text-slate-900 dark:text-slate-100 placeholder-slate-400"
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={3}
                  className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-blue-500 dark:focus:ring-blue-600 focus:border-transparent transition-all outline-none text-slate-900 dark:text-slate-100 resize-none placeholder-slate-400"
                ></textarea>
              </div>
              ...
              <button
                ...
                className="w-full py-3 px-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-xl shadow-md shadow-blue-500/20 transition-all focus:outline-none focus:ring-4 focus:ring-blue-500/30 disabled:opacity-50 disabled:cursor-not-allowed mt-2"
              >
        ...
        <div className="lg:col-span-8">
          <div className="bg-white dark:bg-slate-950 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden flex flex-col h-full">
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-sm text-left">
                <thead className="text-xs text-slate-500 dark:text-slate-400 uppercase bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-800">
                  <tr>
                    <th scope="col" className="px-6 py-4 whitespace-nowrap">사용일자</th>
                    <th scope="col" className="px-6 py-4 whitespace-nowrap">사용처</th>
                    <th scope="col" className="px-6 py-4 whitespace-nowrap text-right">금액</th>
                    <th scope="col" className="px-6 py-4 whitespace-nowrap">사유</th>
                    <th scope="col" className="px-6 py-4 whitespace-nowrap text-center">상태</th>
                  </tr>
                </thead>
                  ...
                  {isLoading ? (
                    ...
                    expenses.map((expense) => (
                      <tr key={expense.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors group">
                        <td className="px-6 py-4 whitespace-nowrap text-slate-600 dark:text-slate-400 tabular-nums">
                          {expense.usage_date}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap font-medium text-slate-900 dark:text-slate-200">
                          {expense.merchant}
                        </td>
                        <td className="px-6 py-4 whitespace-nowrap text-right font-medium text-slate-900 dark:text-slate-200 tabular-nums">
                          {expense.amount.toLocaleString()}원
                        </td>
                        <td className="px-6 py-4 text-slate-600 dark:text-slate-400">
                          <div className="truncate max-w-[200px]">{expense.purpose}</div>
                        </td>
                        <td className="px-6 py-4 text-center whitespace-nowrap">
                          <span className={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-semibold border ${expense.status === '승인'
                            ? 'bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400 border-green-200 dark:border-green-800/50'
                            : expense.status === '반려' || expense.status?.includes('반려')
